const fs = require('fs');

const bestPictureWinners = [
  {
    name: 'Wings',
    awardNumber: 1,
    releaseYear: 1927,
    seen: false,
  },
  {
    name: 'The Broadway Melody',
    awardNumber: 2,
    releaseYear: 1928,
    seen: false,
  },
  {
    name: 'All Quiet On the Western Front',
    awardNumber: 3,
    releaseYear: 1930,
    seen: false,
  },
  {
    name: 'Cimarron',
    awardNumber: 4,
    releaseYear: 1931,
    seen: false,
  },
  {
    name: 'Grand Hotel',
    awardNumber: 5,
    releaseYear: 1932,
    seen: false,
  },
  {
    name: 'Cavalcade',
    awardNumber: 6,
    releaseYear: 1933,
    seen: false,
  },
  {
    name: 'It Happened One Night',
    awardNumber: 7,
    releaseYear: 1934,
    seen: false,
  },
  {
    name: 'Mutiny on the Bounty',
    awardNumber: 8,
    releaseYear: 1935,
    seen: false,
  },
  {
    name: 'The Great Ziegfeld',
    awardNumber: 9,
    releaseYear: 1936,
    seen: false,
  },
  {
    name: 'The Life of Emile Zola',
    awardNumber: 10,
    releaseYear: 1937,
    seen: false,
  },
  {
    name: "You Can't Take It With You",
    awardNumber: 11,
    releaseYear: 1938,
    seen: false,
  },
  {
    name: 'Gone With The Wind',
    awardNumber: 12,
    releaseYear: 1939,
    seen: false,
  },
  {
    name: 'Rebecca',
    awardNumber: 13,
    releaseYear: 1940,
    seen: false,
  },
  {
    name: 'How Green Was My Valley',
    awardNumber: 14,
    releaseYear: 1941,
    seen: false,
  },
  {
    name: 'Mrs. Miniver',
    awardNumber: 15,
    releaseYear: 1942,
    seen: false,
  },
  {
    name: 'Casablanca',
    awardNumber: 16,
    releaseYear: 1942,
    seen: false,
  },
  {
    name: 'Going My Way',
    awardNumber: 17,
    releaseYear: 1944,
    seen: false,
  },
  {
    name: 'The Lost Weekend',
    awardNumber: 18,
    releaseYear: 1945,
    seen: false,
  },
  {
    name: 'The Best Years of Our Lives',
    awardNumber: 19,
    releaseYear: 1946,
    seen: false,
  },
  {
    name: "Gentlemen's Agreement",
    awardNumber: 20,
    releaseYear: 1947,
    seen: false,
  },
  {
    name: 'Hamlet',
    awardNumber: 21,
    releaseYear: 1948,
    seen: false,
  },
  {
    name: "All the King's Men",
    awardNumber: 22,
    releaseYear: 1949,
    seen: false,
  },
  {
    name: 'All About Eve',
    awardNumber: 23,
    releaseYear: 1950,
    seen: false,
  },
  {
    name: 'An American in Paris',
    awardNumber: 24,
    releaseYear: 1951,
    seen: false,
  },
  {
    name: 'The Greatest Show on Earth',
    awardNumber: 25,
    releaseYear: 1952,
    seen: false,
  },
  {
    name: 'From Here to Eternity',
    awardNumber: 26,
    releaseYear: 1953,
    seen: false,
  },
  {
    name: 'On the Waterfront',
    awardNumber: 27,
    releaseYear: 1954,
    seen: false,
  },
  {
    name: 'Marty',
    awardNumber: 28,
    releaseYear: 1955,
    seen: false,
  },
  {
    name: 'Around the World in Eighty Days',
    awardNumber: 29,
    releaseYear: 1956,
    seen: false,
  },
  {
    name: 'The Bridge on the River Kwai',
    awardNumber: 30,
    releaseYear: 1957,
    seen: false,
  },
  {
    name: 'Gigi',
    awardNumber: 31,
    releaseYear: 1958,
    seen: false,
  },
  {
    name: 'Ben-Hur',
    awardNumber: 32,
    releaseYear: 1959,
    seen: false,
  },
  {
    name: 'The Apartment',
    awardNumber: 33,
    releaseYear: 1960,
    seen: false,
  },
  {
    name: 'West Side Story',
    awardNumber: 34,
    releaseYear: 1961,
    seen: false,
  },
  {
    name: 'Lawrence of Arabia',
    awardNumber: 35,
    releaseYear: 1961,
    seen: false,
  },
  {
    name: 'Tom Jones',
    awardNumber: 36,
    releaseYear: 1963,
    seen: false,
  },
  {
    name: 'My Fair Lady',
    awardNumber: 37,
    releaseYear: 1963,
    seen: false,
  },
  {
    name: 'The Sound of Music',
    awardNumber: 38,
    releaseYear: 1965,
    seen: false,
  },
  {
    name: 'A Man for All Seasons',
    awardNumber: 39,
    releaseYear: 1966,
    seen: false,
  },
  {
    name: 'In the Heat of the Night',
    awardNumber: 40,
    releaseYear: 1967,
    seen: false,
  },
  {
    name: 'Oliver!',
    awardNumber: 41,
    releaseYear: 1968,
    seen: false,
  },
  {
    name: 'Midnight Cowboy',
    awardNumber: 42,
    releaseYear: 1969,
    seen: false,
  },
  {
    name: 'Patton',
    awardNumber: 43,
    releaseYear: 1970,
    seen: false,
  },
  {
    name: 'The French Connection',
    awardNumber: 44,
    releaseYear: 1971,
    seen: false,
  },
  {
    name: 'The Godfather',
    awardNumber: 45,
    releaseYear: 1972,
    seen: false,
  },
  {
    name: 'The Sting',
    awardNumber: 46,
    releaseYear: 1973,
    seen: false,
  },
  {
    name: 'The Godfather: Part II',
    awardNumber: 47,
    releaseYear: 1974,
    seen: false,
  },
  {
    name: "One Flew Over the Cuckoo's Nest",
    awardNumber: 48,
    releaseYear: 1975,
    seen: false,
  },
  {
    name: 'Rocky',
    awardNumber: 49,
    releaseYear: 1976,
    seen: false,
  },
  {
    name: 'Annie Hall',
    awardNumber: 50,
    releaseYear: 1977,
    seen: false,
  },
  {
    name: 'The Deer Hunter',
    awardNumber: 51,
    releaseYear: 1978,
    seen: false,
  },
  {
    name: 'Kramer vs. Kramer',
    awardNumber: 52,
    releaseYear: 1979,
    seen: false,
  },
  {
    name: 'Ordinary People',
    awardNumber: 53,
    releaseYear: 1980,
    seen: false,
  },
  {
    name: 'Chariots of Fire',
    awardNumber: 54,
    releaseYear: 1981,
    seen: false,
  },
  {
    name: 'Gandhi',
    awardNumber: 55,
    releaseYear: 1982,
    seen: false,
  },
  {
    name: 'Terms of Endearment',
    awardNumber: 56,
    releaseYear: 1983,
    seen: false,
  },
  {
    name: 'Amadeus',
    awardNumber: 57,
    releaseYear: 1984,
    seen: false,
  },
  {
    name: 'Out of Africa',
    awardNumber: 58,
    releaseYear: 1985,
    seen: false,
  },
  {
    name: 'Platoon',
    awardNumber: 59,
    releaseYear: 1986,
    seen: false,
  },
  {
    name: 'The Last Emperor',
    awardNumber: 60,
    releaseYear: 1987,
    seen: false,
  },
  {
    name: 'Rain Man',
    awardNumber: 61,
    releaseYear: 1988,
    seen: false,
  },
  {
    name: 'Driving Miss Daisy',
    awardNumber: 62,
    releaseYear: 1988,
    seen: false,
  },
  {
    name: 'Dances with Wolves',
    awardNumber: 63,
    releaseYear: 1990,
    seen: false,
  },
  {
    name: 'The Silence of the Lambs',
    awardNumber: 64,
    releaseYear: 1991,
    seen: false,
  },
  {
    name: 'Unforgiven',
    awardNumber: 65,
    releaseYear: 1992,
    seen: false,
  },
  {
    name: "Schindler's List",
    awardNumber: 66,
    releaseYear: 1993,
    seen: false,
  },
  {
    name: 'Forest Gump',
    awardNumber: 67,
    releaseYear: 1994,
    seen: false,
  },
  {
    name: 'Braveheart',
    awardNumber: 68,
    releaseYear: 1995,
    seen: false,
  },
  {
    name: 'The English Patient',
    awardNumber: 69,
    releaseYear: 1996,
    seen: false,
  },
  {
    name: 'Titanic',
    awardNumber: 70,
    releaseYear: 1997,
    seen: false,
  },
  {
    name: 'Shakespeare in Love',
    awardNumber: 71,
    releaseYear: 1998,
    seen: false,
  },
  {
    name: 'American Beauty',
    awardNumber: 72,
    releaseYear: 1999,
    seen: false,
  },
  {
    name: 'Gladiator',
    awardNumber: 73,
    releaseYear: 2000,
    seen: false,
  },
  {
    name: 'A Beautiful Mind',
    awardNumber: 74,
    releaseYear: 2001,
    seen: false,
  },
  {
    name: 'Chicago',
    awardNumber: 75,
    releaseYear: 2002,
    seen: false,
  },
  {
    name: 'The Lord of the Rings: The Return of the King',
    awardNumber: 76,
    releaseYear: 2003,
    seen: false,
  },
  {
    name: 'Million Dollar Baby',
    awardNumber: 77,
    releaseYear: 2004,
    seen: false,
  },
  {
    name: 'Crash',
    awardNumber: 78,
    releaseYear: 2004,
    seen: false,
  },
  {
    name: 'The Departed',
    awardNumber: 79,
    releaseYear: 2006,
    seen: false,
  },
  {
    name: 'No Country for Old Men',
    awardNumber: 80,
    releaseYear: 2007,
    seen: false,
  },
  {
    name: 'Slumdog Millionaire',
    awardNumber: 81,
    releaseYear: 2008,
    seen: false,
  },
  {
    name: 'The Hurtlocker',
    awardNumber: 82,
    releaseYear: 2008,
    seen: false,
  },
  {
    name: "The King's Speech",
    awardNumber: 83,
    releaseYear: 2010,
    seen: false,
  },
  {
    name: 'The Artist',
    awardNumber: 84,
    releaseYear: 2011,
    seen: false,
  },
  {
    name: 'Argo',
    awardNumber: 85,
    releaseYear: 2012,
    seen: false,
  },
  {
    name: '12 Years a Slave',
    awardNumber: 86,
    releaseYear: 2013,
    seen: false,
  },
  {
    name: 'Birdman, or The Unexpected Virtue of Ignorance',
    awardNumber: 87,
    releaseYear: 2014,
    seen: false,
  },
  {
    name: 'Spotlight',
    awardNumber: 88,
    releaseYear: 2015,
    seen: false,
  },
  {
    name: 'Moonlight',
    awardNumber: 89,
    releaseYear: 2016,
    seen: false,
  },
  {
    name: 'The Shape of Water',
    awardNumber: 90,
    releaseYear: 2017,
    seen: false,
  },
];

bestPictureWinners.forEach((picture, index) => {
  const obj = {
    title: picture.name,
    releaseYear: picture.releaseYear.toString(),
    awardYear: (picture.releaseYear + 1).toString(),
    awardNumber: picture.awardNumber.toString(),
    nominees: [],
  };
  const data = JSON.stringify(obj);
  fs.writeFileSync(`./movies/${index + 1}.json`, data);
});
